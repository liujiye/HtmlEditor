package com.ljy.htmleditor.html;
...
import com.ljy.htmleditor.html.tagsoup.HTMLSchema;
import com.ljy.htmleditor.html.tagsoup.Parser;
...
public class Html
{
	...
	public static Spanned fromHtml(String source, ImageGetter imageGetter, TagHandler tagHandler)
	{
		...
		HtmlToSpannedConverter converter = new HtmlToSpannedConverter(source, imageGetter,
				tagHandler, parser);
			...
			for (int j = 0; j < style.length; j++)
			{
				if (style[j] instanceof AlignmentSpan)
				{
					...
					if (align == Layout.Alignment.ALIGN_CENTER)
					{
						...
						elements = "style=\"text-align: center;\" " + elements;
					}
					else if (align == Layout.Alignment.ALIGN_OPPOSITE)
					{
						// elements = "align=\"right\" " + elements;
						elements = "style=\"text-align: right;\" " + elements;
					}
					else
					{
						// elements = "align=\"left\" " + elements;
						// elements = "style=\"left\" " + elements;
					}
				}
			}
			if (needDiv)
		...
		for (int i = start; i < end; i = next)
		{
			...
			for (int j = 0; j < style.length; j++)
			{
				...
				if (style[j] instanceof ForegroundColorSpan)
				{
					...
					String color = Integer.toHexString(((ForegroundColorSpan) style[j])
...
class HtmlToSpannedConverter implements ContentHandler
{
	...
	private void handleStartTag(String tag, Attributes attributes)
	{
		...
		else if (tag.equalsIgnoreCase("p"))
		{
			...
			startPStyle(mSpannableStringBuilder, attributes);
		}
		...
		else if (tag.length() == 2 && Character.toLowerCase(tag.charAt(0)) == 'h'
	...
	private void handleEndTag(String tag)
	{
		...
		{
			handleP(mSpannableStringBuilder);
			endPStyle(mSpannableStringBuilder);
		}
		else if (tag.equalsIgnoreCase("div"))
		{
			handleP(mSpannableStringBuilder);
	...
	private static void startPStyle(SpannableStringBuilder text, Attributes attributes)
	{
		if (attributes != null)
		{
			...
			style = style.trim();
			int len = text.length();
			if ("text-align: right;".equalsIgnoreCase(style))
			{
				text.setSpan(new AlignmentSpan.Standard(Layout.Alignment.ALIGN_OPPOSITE), len, len,
						Spannable.SPAN_MARK_MARK);
			}
			else if ("text-align: center;".equalsIgnoreCase(style))
			{
				text.setSpan(new AlignmentSpan.Standard(Layout.Alignment.ALIGN_CENTER), len, len,
						Spannable.SPAN_MARK_MARK);
			}
			else
			{
	...
	private static void endPStyle(SpannableStringBuilder text)
	{
		int len = text.length();
		Object obj = getLast(text, AlignmentSpan.Standard.class);
		int spanStart = text.getSpanStart(obj);
		int spanEnd = text.getSpanEnd(obj);

		if (spanStart != len && spanStart == spanEnd && obj != null)
		{
			text.removeSpan(obj);

			AlignmentSpan.Standard align = (AlignmentSpan.Standard) obj;

			text.setSpan(new AlignmentSpan.Standard(align.getAlignment()), spanStart, len,
			...
			Html.ImageGetter img)
	{
		...
		if (d == null)
		{
			d = Resources.getSystem().getDrawable(android.R.drawable.arrow_down_float);
	...
	private static void startFont(SpannableStringBuilder text, Attributes attributes)
	{
		...
		// Log.d("startFont", "color : " + color + "; face : " + face +
		// "; size : " + size
		// + "; text : " + text);
	...
	private static void endFont(SpannableStringBuilder text)
	{
		...
		if (where != len)
		{
			...
			{
				String size = f.mSize.toLowerCase().trim();
				if (size.endsWith("px"))
				{
					size = size.substring(0, size.indexOf("px")).trim();
				}

				text.setSpan(new AbsoluteSizeSpan(Integer.valueOf(size), false), where, len,
						Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
			}
		}
	}
