...
public class CommandLine
{
	...
	static
	{
		...
		options.put("--files", Boolean.FALSE); // process arguments as separate
												// files
		...
		options.put("--emptybogons", Boolean.FALSE); // unknowns have EMPTY
														// content model
		options.put("--norootbogons", Boolean.FALSE); // unknowns can't be the
														// root
		...
		options.put("--ignorable", Boolean.FALSE); // return ignorable
													// whitespace
	}
	...
	private static ContentHandler chooseContentHandler(Writer w)
	{
		...
		if (hasOption(options, "--output-encoding="))
		{
			...
			// System.err.println("%%%% Output encoding is " +
			// theOutputEncoding);
			if (theOutputEncoding != null)
			{
