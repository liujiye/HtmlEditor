...
public class Parser extends DefaultHandler implements ScanHandler, XMLReader, LexicalHandler
{
	...
	private String expandEntities(String src)
	{
		...
		for (int i = 0; i < len; i++)
		{
			...
			else if (ch == ';')
			{
				...
				// System.err.print("got [" + new String(dst, refStart,
				// dstlen-refStart-1) + "]");
	...
	private int lookupEntity(char[] buff, int offset, int length)
	{
		...
		// System.err.println("%% Got entity [" + new String(buff, offset,
		// length) + "]");
		if (buff[offset] == '#')
		{
	...
	private void pop() throws SAXException
	{
		...
		if (foreign(prefix, namespace))
		{
			...
			// System.err.println("%% Unmapping [" + prefix +
		...
		for (int i = atts.getLength() - 1; i >= 0; i--)
		{
			...
			if (foreign(attPrefix, attNamespace))
			{
				...
				// System.err.println("%% Unmapping [" + attPrefix +
	...
	private void push(Element e) throws SAXException
	{
		...
		if (foreign(prefix, namespace))
		{
			...
			// System.err.println("%% Mapping [" + prefix + "] for elements to "
			// + namespace);
		}
		...
		for (int i = 0; i < len; i++)
		{
			...
			if (foreign(attPrefix, attNamespace))
			{
				...
				// System.err.println("%% Mapping [" + attPrefix +
				// "] for attributes to " + attNamespace);
			}
		}
	...
	{
		// System.err.print("%% Testing " + prefix + " and " + namespace +
		// " for foreignness -- ");
		boolean foreign = !(prefix.equals("") || namespace.equals("") || namespace.equals(theSchema
				.getURI()));
	...
	public void pi(char[] buff, int offset, int length) throws SAXException
	{
		...
		// System.err.println("%% Removing ? from PI");
		if (length > 0 && buff[length - 1] == '?')
	...
	private void rectify(Element e) throws SAXException
	{
		...
		while (true)
		{
			...
			// System.err.println("%% Ascending from " + e.name() + " to " +
			// parent.name());
			parent.setNext(e);
			e = parent;
		}
